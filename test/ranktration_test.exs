    ...
    test "demonstrates performance gradients with 8+ trajectories" do
      ...
      trajectories = [
        ...
        TrajectoryResult.new(
          "precision_expert",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.95, speed: 0.4, robustness: 0.7)
        ),
        # Speed Demon: Fast but inaccurate
        TrajectoryResult.new(
          "speed_demon",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.6, speed: 0.95, robustness: 0.5)
        ),
        # All-Rounder: Balanced performer
        TrajectoryResult.new(
          "all_rounder",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.8, speed: 0.8, robustness: 0.8)
        ),
        # Robust Specialist: Resilient but average
        TrajectoryResult.new(
          "robust_specialist",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.7, speed: 0.6, robustness: 0.95)
        ),
        # Efficiency Focus: Good accuracy/speed balance
        TrajectoryResult.new(
          "efficiency_focus",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.75, speed: 0.85, robustness: 0.7)
        ),
        # Underperformer: Poor across all metrics
        TrajectoryResult.new(
          "underperformer",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.5, speed: 0.5, robustness: 0.4)
        ),
        # Mixed Bag: Strong speed, decent accuracy
        TrajectoryResult.new(
          "mixed_bag",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.7, speed: 0.9, robustness: 0.6)
        ),
        # Elite Hybrid: Near-perfect across all
        TrajectoryResult.new(
          "elite_hybrid",
          "gradient_test",
          Ranktration.Metrics.new(accuracy: 0.9, speed: 0.9, robustness: 0.8)
        )
      ]
      ...
      accuracy_first =
        RulerCore.new(metric_weights: %{"accuracy" => 0.7, "speed" => 0.2, "robustness" => 0.1})

      result_accuracy =
        RulerCore.evaluate_trajectories(accuracy_first, trajectories, "gradient_test")
      ...
      # Elite hybrid beats precision when accuracy is weighted heavily
      assert elite_hybrid_idx < precision_idx
      # Precision still beats speed
      assert precision_idx < speed_idx
      # Speed beats underperformer
      assert speed_idx < underperformer_idx
      ...
      speed_first =
        RulerCore.new(metric_weights: %{"speed" => 0.7, "accuracy" => 0.2, "robustness" => 0.1})
      ...
      # Elite hybrid beats speed demon when speed is weighted heavily
      assert elite_hybrid_idx < speed_idx
      elite_hybrid_score = result_speed.scores["elite_hybrid"]
      speed_score = result_speed.scores["speed_demon"]
      # Elite hybrid has higher speed-weighted score
      assert elite_hybrid_score > speed_score
      ...
      balanced =
        RulerCore.new(
          metric_weights: %{"accuracy" => 0.33, "speed" => 0.33, "robustness" => 0.34}
        )
      ...
      # Elite hybrid beats all-rounder in balanced scoring
      assert elite_idx_balanced < all_rounder_idx
      ...
      # Even underperformer beats worst performers
      assert scores["underperformer"] < scores["precision_expert"]
